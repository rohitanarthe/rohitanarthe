...
class DealerHistoryCardWidget extends StatelessWidget {
  final List<ViewDealerVisitList> visits;

  DealerHistoryCardWidget({required this.visits});
        ...
        return Container(
          ...
          child: DataTable(
            ...
            rows: visits.asMap().entries.map((entry) {
              int index = entry.key;
              ViewDealerVisitList visit = entry.value;

              return DataRow(cells: [
                DataCell(Text((index + 1).toString(), style: TextStyle(fontSize: isSmallScreen ? 10 : 10))),
                DataCell(Text(visit.firmName ?? '', style: TextStyle(fontSize: isSmallScreen ? 10 : 12))),
                DataCell(Text(visit.custMobNo ?? '', style: TextStyle(fontSize: isSmallScreen ? 10 : 12))),
                DataCell(Text(visit.custFollowUpDate ?? '', style: TextStyle(fontSize: isSmallScreen ? 10 : 12))),
              ]);
            }).toList(),
          ),
        );
